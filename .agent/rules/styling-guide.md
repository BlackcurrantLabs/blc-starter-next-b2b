...
description: Use this when theming / styling elements
---

Go for a modern clean design. Don't over-use cards.
Have a clear focus point when designing UI/UX, don't put in too many things that take up user's attention.

Rules:
Use shadcn components where possible.
Use radix primitives if a direct shadcn equivalent is not available and needs to be constructed.
avoid editing components in components/ directory, instead style them with local styling.
